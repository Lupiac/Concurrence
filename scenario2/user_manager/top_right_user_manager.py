...
class TopRightUserMangager(threading.Thread):
    ...
    def __init__(self, users, cases, events, finish_event, drawer, draw_is_enable):
        ...
        self.bottom_right_manager = None
    ...
    def set_bottom_right_manager(self, bottom_right_manager):
        self.top_right_manager = bottom_right_manager
    ...
    def give_user(self, user):
        ...
        if user.X < constante.WIDTH / 2 and user.Y >= constante.HEIGHT / 2:
            ...
            self.bottom_right_manager.users.append(user)
            self.events[3].set()
